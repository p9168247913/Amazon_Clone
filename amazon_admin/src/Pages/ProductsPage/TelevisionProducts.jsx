...
export const TelevisionProducts = () => {
  ...
  const getTelivisionAPI = async () => {
    setLoading(true)
    let data = await getDataAPI("telivision")
    setLoading(false)
  ...
  const handelDelete = (value) =>{
    // deleteAPI("telivision", value)
    axios.delete(`https://atmazon.onrender.com/telivision/${value}`)

    getTelivisionAPI()
  }

  const handlePatch = (value) =>{
    editAPI("telivision", value)
    getTelivisionAPI()
  }
  ...
  return (
    <>
      <div style={{ display: "flex", justifyContent: "flex-end", alignItems: "center", padding: "1rem" }}>
        <div>
          <Link to="/add/telivision" style={{ padding: "10px 20px", cursor: "pointer", textDecoration:"none", backgroundColor: "teal", color: "#fff", fontSize: "18px", borderRadius: "5px" }}>Add New</Link>
        </div>
      </div>
