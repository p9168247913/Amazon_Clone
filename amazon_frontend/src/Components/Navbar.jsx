import { Button, Input } from "@chakra-ui/react";
import { useSelector } from "react-redux";
import { Link } from "react-router-dom";
import { logout } from "../Redux/AuthReducer/action";
import { SearchIcon } from '@chakra-ui/icons'
import "./navbar.css"
function Navbar() {
    const isAuth = useSelector(store => store.authReducer.isAuth);
    return (
        <div id="nav" style={{
            display: "flex", justifyContent: "space-around",
            padding: "15px",
            background: "#131921"
        }}>
            <Link to="/"><img src="https://zeevector.com/wp-content/uploads/LOGO/Amazon-India-Logo-PNG-White2.png"
                alt="amazon" style={{ width: "100px", background: "#131921" }} /></Link>

            <Link to=""><img src="data:image/png;base64,"
                alt="location" style={{ height: "15px", width: "20px", }} />
                <select name="Location" id="" style={{ border: "0", background: "#131921", color: "whitesmoke" }}>
                    <option value="selelct">Select</option>
                    <option value="Hyderabad">Hyderabad</option>
                    <option value="Chennai">Chennai</option>
                    <option value="bangaloore">Bangalore</option>
                    <option value="delhi">Delhi</option>
                    <option value="mumbai">Mumbai</option>
                    <option value="gujarat">Gujarat</option>
                </select> </Link>

            {/* categories Section start  point */}

            <Link to="/categories" style={{ color: "white" }}>

                Categories

            </Link>

            {/* categories Section End Part */}


            <Link id="searchBarr" >
                <Input type={"search"} color={"white"} />
                <SearchIcon bg={"#febd69"} height={"40px"} />
            </Link>


            {/* SearchBar Functionality */}

            <Link to="/language" style={{ color: "white" }}>Language</Link>
            {isAuth ? <Link style={{ color: "white" }} onClick={() => logout()}>Sign Out</Link> :
                <Link to="/login" style={{ color: "white" }}>Login</Link>
            }
            <Link to="/admin" style={{ color: "white" }} >Admin Pannel</Link>
            <Link to="/returns" style={{ color: "white" }}>Returns</Link>


            <Link to="/cart" style={{ color: "white" }} >
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT0tBS73GPTm5sRcq_9NrB5EPOleQM5bbEEUtn042wk4u2Fc6GcpXr8rRyfI1JjyGNR1fE&usqp=CAU"
                    style={{ height: "25px", width: "25px", marginTop: "-10px", color: "white", background: "#131921" }} alt="cart" />
                Cart
            </Link>
        </div>

    )
}
export default Navbar;